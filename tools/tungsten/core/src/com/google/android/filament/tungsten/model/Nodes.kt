...
private val shaderNodeCompile = fun(node: Node, compiler: GraphCompiler): Node {
    val baseColorSlot = node.getInputSlot("baseColor")
    val baseColor = compiler.compileAndRetrieveVariable(baseColorSlot)
...
object GraphInitializer {
    ...
    fun getInitialGraphState(): Graph {
        ...
        return Graph(nodes = listOf(shaderNode)).graphBySettingRootNode(shaderNode)
    }
}
